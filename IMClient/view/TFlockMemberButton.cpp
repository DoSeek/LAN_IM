...
IMFlockMemberButton::IMFlockMemberButton(const TFlockMember &flockMember,
                                         TFlockChatWidget *flockWidget,
                                         TMainWidget *mainWidget, QWidget *parent) :
    QPushButton(parent)
{
    m_info = flockMember;
...
void IMFlockMemberButton::setButtonStatus()
{
    ...
    if(ELS_OFFLINE == m_info.m_loginStatus || INVISIBLE == m_info.m_loginStatus)
    {
        QPixmap pixmap;
        QIcon icon(str);
...
{
    bool isOk = false;
    QString remark = QInputDialog::getText(nullptr, "修改群名片",
                                           "请输入新的群名片",
                                           QLineEdit::Normal,
                                           m_info.m_remark,
                                           &isOk);
...
void IMFlockMemberButton::onClickRemoveMember()
{
    ...
    returnBtn = QMessageBox::question(nullptr, tr("移除群成员"),
...
QString IMFlockMemberButton::getFlockStatusFromInt(const int n) const
{
    ...
    case EFS_FLOCK_HOST:
        return tr("群主");
        break;
    case EFS_FLOCK_ADMAIN:
//        return tr("管理员");
//        break;
    case EFS_FLOCK_MEMBER:
        return tr("成员");
        break;
    default:
        break;
