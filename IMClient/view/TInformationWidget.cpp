...
TInformationWidget::TInformationWidget(TMainWidget *mainWidget, QWidget *parent) :
    QWidget(parent)
{
...
void TInformationWidget::setReadOnly(bool isReadOnly, bool isEdit)
{
...
//void TInformationWidget::setWrite()
//{}

void TInformationWidget::setUserInformation(const TUserInformation &user)
{
    m_userInf = user;
...
TUserInformation TInformationWidget::getInformation()
{
//    m_userInf.m_userID
    //            .getDate()
...
void TInformationWidget::closeEvent(QCloseEvent *)//离开时触发的  应该检查窗口是否都关闭了！！
{
...
// public slots:---------------------------------
void TInformationWidget::onClickCancelButton()
{
    if (!m_isReadOnly)
    {
...
void TInformationWidget::onClickOKButton()
{
    if (!m_isReadOnly)
    {
        getInformation();
...
void TInformationWidget::onClickPretButton()
{
    --m_headNum;
    if(m_headNum < 1)
        m_headNum = MAX_NUM;
...
void TInformationWidget::onClickNextButton()
{
    ++m_headNum;
...
// private:----------------------------------------
void TInformationWidget::initIMLoginWidget()
{
    m_isReadOnly = true;
    m_isEdit = false;
