...
TDiscussionButton::TDiscussionButton(const TDiscussionInformation &info,
...
TDiscussionButton::~TDiscussionButton()
{
}
...
Description: 打开聊天窗口
*************************************************/
void TDiscussionButton::openChatRoom()
{
    if (m_mainWidget == nullptr)
        return;
...
//        m_discussionRoom->show();
    ...
    TDiscussionChatWidget *chatRoom = new TDiscussionChatWidget(m_discussionInfo,
                                                                  m_mainWidget);
    ...
    connect(m_mainCtrl, SIGNAL(getDiscussionMembersSuccess(QVector<TDiscussionMember>)),
            this, SLOT(setDiscussionMemberList(QVector<TDiscussionMember>)));
    connect(m_mainCtrl, SIGNAL(getDiscussionMembersSuccess(QVector<TDiscussionMember>)),
            chatRoom, SLOT(setDiscussionMemberList(QVector<TDiscussionMember>)));
...
Description: 显示聊天窗口
*************************************************/
void TDiscussionButton::showChatRoom()
{
    m_discussionRoom->raise();
    m_discussionRoom->activateWindow();
    m_discussionRoom->show();
...
Description: 关闭聊天窗口
*************************************************/
void TDiscussionButton::closeChatRoom()
{
    if(m_isOpen)
    {
...
*************************************************/
void TDiscussionButton::setButtonStatus()
{
...
Description: 删除成员
*************************************************/
void TDiscussionButton::deleteDiscussionMember(const QString & memberID)
{
...
Description: 添加成员
*************************************************/
void TDiscussionButton::addDiscussionMember(const QString & memberID)
{
...
Description: 左键 双击
*************************************************/
void TDiscussionButton::onDoubleClickDiscussionButton()
{
    openChatRoom();
    showChatRoom();
...
Description: 右键点击按键
*************************************************/
void TDiscussionButton::onClickRightButton()
{
    if (m_menu == nullptr)
    {
        //创建右键菜单
...
Description: 发送群消息
*************************************************/
void TDiscussionButton::onClickSendMessage()
{
    openChatRoom();
}
...
Description: 查看消息记录
*************************************************/
void TDiscussionButton::onClickShowRecord()
{
}
...
Description: 解散讨论组
*************************************************/
void TDiscussionButton::onClickDropDiscussion()
{
    QMessageBox::StandardButton returnBtn;

    returnBtn = QMessageBox::question(nullptr, tr("解散讨论组"),
...
Description: 退出讨论组
*************************************************/
void TDiscussionButton::onClickLeaveDiscussion()
{
    QMessageBox::StandardButton returnBtn;

    returnBtn = QMessageBox::question(nullptr, tr("讨论组"),
...
Description: 添加成员
*************************************************/
void TDiscussionButton::onClickAddMember()
{
    bool isOk = false;
    QString memberID = QInputDialog::getText(nullptr, "添加讨论组成员",
                                           "请输入对方帐号",
                                           QLineEdit::Normal,
                                           "",
                                           &isOk);
    ...
    for (int i =0; i<m_discussionMembersVec.size(); ++i)
    {
        if (0 == memberID.compare(m_discussionMembersVec[i].m_userID))
        {
            QMessageBox::critical(nullptr, tr("添加失败"), tr("该用户已经是该讨论组的成员"));
            return;
        }
    }
...
Description: 显示详细信息
*************************************************/
void TDiscussionButton::onClickShowInformation()
{
    if (m_mainWidget == nullptr)
        return;
...
Description: 聊天窗口关闭了
*************************************************/
void TDiscussionButton::chatRoomQuit()
{
    m_isOpen = false;
    m_isShow = false;
    if (nullptr != m_mainWidget)
...
Description: 设置成员列表
*************************************************/
void TDiscussionButton::setDiscussionMemberList(const QVector<TDiscussionMember> & memberList)
{
...
Description: 鼠标单击事件
*************************************************/
void TDiscussionButton::mousePressEvent(QMouseEvent *event)
{
...
Description: 鼠标双击事件
*************************************************/
void TDiscussionButton::mouseDoubleClickEvent(QMouseEvent *event)
{
...
Description: 创建右键菜单
*************************************************/
void TDiscussionButton::creatMenu(/*const QList<QString> *groutList*/)
{
