...
void IMMailWriteWidget::onClickBtnSend()
{
    ...
    {
        QMessageBox::about(nullptr, tr("提示"), tr("收件人不能为空！"));
        return;
    }
    if (m_leReceiverAddress->text().compare(m_myID) == 0)
    {
        QMessageBox::about(nullptr, tr("提示"), tr("不能给自己发送邮件！"));
        return;
    }
    if (m_leTheme->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("信件主题不能为空"));

        return;
    }
    if (m_textInput->toPlainText().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("信件内容不能为空！"));
        return;
    }
