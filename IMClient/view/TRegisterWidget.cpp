...
void IMRegisterWidget::onClickBtnRegister()
{
    ...
    {
        QMessageBox::about(nullptr, tr("提示"), tr("昵称不能为空"));
        return;
    }
    // 判断密码是否为空
    if (m_lePwd->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("密码不能为空"));
        return;
    }
    // 判断密码是否符合要求
    if (m_lePwd->text().length()>14 || m_lePwd->text().length()<6)
    {
        QMessageBox::about(nullptr, tr("提示"), tr("密码长度不符合"));
        return;
    }
    // 判断确认密码是否为空
    if (m_leConfirmPwd->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("请确认密码"));
        return;
    }
    // 判断密码是否一致
    if (m_lePwd->text().compare(m_leConfirmPwd->text()) != 0)
    {
        QMessageBox::about(nullptr, tr("提示"), tr("密码不一致"));
        return;
    }
    // 判断性别是否为空
    if (m_cbxSex->currentText().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("性别不能为空"));
        return;
    }
    // 判断生日是否为空
    if (m_leBirthday->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("生日不能为空"));
        return;
    }
    // 判断密保问题是否为空
    if (m_leQuestion->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("密保不能为空"));
        return;
    }
    // 判断问题答案是否为空
    if (m_leAnswer->text().isEmpty())
    {
        QMessageBox::about(nullptr, tr("提示"), tr("问题答案不能为空"));
        return;
    }
